...
public class ToastTechnology implements Toast {
	...
	public Visibility render(GuiGraphics graphics, ToastComponent toastGui, long delta) {
		...
		if (Minecraft.getInstance().font.width(title) <= 125) {
			graphics.drawString(Minecraft.getInstance().font, display, 30, 7, 0xFFFF00);
			graphics.drawString(Minecraft.getInstance().font, title, 30, 18, -1);
		} else {
			int alpha;
			if (delta < 1500L) {
				alpha = Mth.floor(Mth.clamp((float) (1500L - delta) / 300.0F, 0.0F, 1.0F) * 255.0F) << 24 | 0x400000;
				graphics.drawString(Minecraft.getInstance().font, display, 30, 11, 0xFFFF00 | alpha);
			} else {
				alpha = Mth.floor(Mth.clamp((float) (delta - 1500L) / 300.0F, 0.0F, 1.0F) * 252.0F) << 24 | 0x400000;
				var lines = Minecraft.getInstance().font.split(Component.literal(title), 125);
				int y = 8;
				for (var line : lines) {
					graphics.drawString(Minecraft.getInstance().font, line, 30, y, 0xFFFFFF | alpha);
					y += 10;
				}
			}
		}
