...
import com.fuxingcheng.fromthegroundup.api.technology.puzzle.PuzzleConnect;
...
public class ContainerResearchTable extends ContainerResearch {
	...
	public ContainerResearchTable(int containerId, TileEntityResearchTable tileEntity, Inventory invPlayer) {
		super(Content.m_researchTable, containerId);
			...
			if (slotIndex == output) {
				...
				if (itemStack2.getItem() == Content.i_parchmentIdea) {
					...
					if (!moveItemStackTo(itemStack2, feather, feather + 1, false))
						return ItemStack.EMPTY;
				} else if (itemStack2.getItem() == Content.i_magnifyingGlass) {
