...
public class EventHandler {
	...
	public void onPlayerJoin(PlayerEvent.PlayerLoggedInEvent evt) {
		if (!evt.getEntity().level().isClientSide()) {
			...
			player.getCapability(CapabilityTechnology.TECH_CAP).ifPresent(cap -> {
	...
	public void onServerTick(TickEvent.ServerTickEvent event) {
		...
		if (event.getServer().getTickCount() % 20 == 0) {
			...
			if (com.Fuxingcheng.ftgumod.criterion.TriggerItemInventory.INSTANCE != null) {
				for (ServerPlayer player : event.getServer().getPlayerList().getPlayers()) {
					com.Fuxingcheng.ftgumod.criterion.TriggerItemInventory.INSTANCE.trigger(player);
				}
			}
		}
	}
