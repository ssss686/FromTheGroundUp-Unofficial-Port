package com.Fuxingcheng.ftgumod.api.util;
...
import com.Fuxingcheng.ftgumod.api.util.predicate.BlockPredicate;
...
public class BlockSerializable {
	...
	public BlockSerializable(CompoundTag compound) {
		...
		if (server != null) {
			...
			// Fallback for client-side parsing
	...
	public CompoundTag serialize() {
		...
		MinecraftServer server = net.minecraftforge.server.ServerLifecycleHooks.getCurrentServer();
		RegistryAccess registryAccess = server != null ? server.registryAccess() : RegistryAccess.fromRegistryOfRegistries(BuiltInRegistries.REGISTRY);
		compound.put("display", display.save(registryAccess));
