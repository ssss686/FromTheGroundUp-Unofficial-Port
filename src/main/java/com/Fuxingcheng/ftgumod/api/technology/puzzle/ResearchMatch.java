package com.Fuxingcheng.ftgumod.api.technology.puzzle;
...
import com.Fuxingcheng.ftgumod.api.technology.recipe.IPuzzle;
...
public class ResearchMatch implements IResearchRecipe {
	...
	public static class Factory implements IResearchRecipe.Factory<ResearchMatch> {
		...
		public ResearchMatch deserialize(JsonObject object, JsonContextPublic context, ResourceLocation technology) {
			...
			for (Map.Entry<String, JsonElement> entry : GsonHelper.getAsJsonObject(object, "key").entrySet()) {
				...
				if (first.has("consume"))
					use = GsonHelper.getAsBoolean(first, "consume");
			...
			JsonArray patternJ = GsonHelper.getAsJsonArray(object, "pattern");
